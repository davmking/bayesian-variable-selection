...
Shrinkage priors are often represented as a **scale mixture of normals**, or a mixture of normal distributions with variances drawn from another distribution.

The Bayesian analogue to LASSO regression places an exponential prior over $\lambda$ on the variance, which results in a Laplace prior for the $\beta$s. 

The Normal-Gamma prior expands on the Bayesian LASSO, placing a gamma prior on the variance and allowing the distribution to have a lot of mass close to zero while maintaining heavy tails for the non-zero coefficients.
...
Horseshoe priors use a global hyperparameter to shrink all coefficients towards zero, while maintaining a local hyperparameter to adjust the scale of shrinkage locally for some coefficients. These are known as **global-local** shrinkage priors. 

Drawing the variance from a half-Cauchy distribution, the model results in a shrinkage coefficient that places a lot of mass near zero (indicating signals) and one (indicating noise), giving the horseshoe prior its name.
...
As an alternative to MCMC sampling, the EMVS algorithm is frequently much more computationally efficient with demonstrated prowess in high-dimensional settings. After initializing Bayesian regression with a spike-and-slab prior, the algorithm repeats two steps until convergence:
...
Researchers have suggested that the EMVS algorithm may perform much better than MCMC when $p >> n$.
...
## Winners
 ...
 - **Response Variable Accuracy:** BMA
